...
function Horror(){
    ...
    return(
        ...
        <div className="pt-32 flex flex-wrap justify-around">
            {
                search?AllMovies.flat().filter(movie=>movie.original_title.toLowerCase().includes(search.toLowerCase()) && movie.genre_ids.includes(27)).map(item=>
                    <MovieCard key={item.id} poster_path={item.poster_path} movie_name={item.original_title} id={item.id}/>
                )
                :
                AllMovies.flat().map(movie=>
                 movie.genre_ids.includes(27)&&<MovieCard id={movie.id} movie_name={movie.original_title} poster_path={movie.poster_path}/>
                )
            }
        </div>
        }
